#below is code for standardized survival and cumulative surviavl models for 2017


# Standarzied Upper Rel Survival Plot ------------------------------------------------
...
ggplot(s10_filtered) + 
  ...
  geom_line(data = s10_lambda, aes(x = rkm, y = Estimate),  color = 'white', size = 1.2, alpha = 0.2) + 
  ...
  annotate("text", label = "Restoration \nArea", x = 300, y = 0, size = 5, lineheight = 0.8, color = "forest green") +
  annotate("text", label = "Lower \nRiver", x = 220, y = 0, size = 5, lineheight = 0.8, color = "purple") +
  annotate("text", label = "Delta", x = 125, y = 0, size = 5, color = "orange") +
  annotate("text", label = "Estuary", x = 20, y = 0, size = 5, color = "blue") +
  
  scale_color_manual(values = c("black", "coral", "steelblue" ), labels = c("Mainstem SJ", "Old River", "Paradise Cut"), 
                     name = "Route") + 
  scale_x_reverse() +
  labs(title = bquote("Standardized Reach-Specific Survival Estimate (" ~S[10]~ ")"), y = expression(S[10]), 
...
ggplot(s10_filtereddelt) + 
  ...
  geom_line(data = s10_lambdadelt, aes(x = rkm, y = Estimate),  color = 'white', size = 1.2, alpha = 0.2) + 
...
cumul_est <- read_csv("data/Survival/datafor_GS_surival_code/cumul_estimates.csv")
...
#Cumul Survival:Delta (written by Colby) (sA1 + BCA to Chipps conv function + Benicia)
delt_cumsurv_conv_2017 <- read_csv("~/GitHub/JSATS_SpringRun17_18/data/2017_delta_cumsurv_bca_chipps_ben.csv")

#delt_cumsurv_conv_2017 <- delt_cumsurv_conv_2017[1:4, ] if you want to eliminate lambda, etc..

#if you want to fix data with spline (dont really understand this)
spline_data <- as.data.frame(spline(delt_cumsurv_conv_2017$rkm, delt_cumsurv_conv_2017$cumul))
ggplot(spline_data) +
  geom_smooth(aes(x = x, y = y)) +
  scale_x_reverse()

# survival without fixing with spline function (THIS ONE USED FOR PRESENATION):


ggplot(data = delt_cumsurv_conv_2017, aes(x= rkm, y = cumul)) +
  geom_smooth(color = "black") +
  scale_x_reverse()+
  theme_classic()+
  geom_vline(xintercept = 72.24, linetype="dotted", 
             color = "black", size=0.5) +
  annotate("text", label = "Delta", x = 125, y = 0, size = 5, color = "black") +
  annotate("text", label = "Estuary", x = 20, y = 0, size = 5, color = "black") +
  labs(title = bquote("Cumulative Survival Estimate"), y = "Survival", 
       x = "River Kilometers from Golden Gate") +
  annotate("text", label = lam, x = 15, y = 0.2, parse = T, size = 4) +
  theme(text = element_text(size = 16)) +
  ggsave("figure_output/Survival/final_plots/delta_2017_cumulsurv.pdf", width = 12, height = 7) 
