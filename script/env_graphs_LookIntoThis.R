# Aug 20, 2018 CH
# Re-tweaking environmental data graphs
...
cdec.dat %>% 
  ...
  labs(title = "Lower Release Discharge", y = "Mean Daily Flow (cfs)", 
       x = "Date") +
  theme_classic() + 
  ...
  ggsave("figure_output/environmental_data/Discharge/updated_Deltaflow.pdf", width = 10, height = 8)
...
cdec.dat %>% 
  ...
  #scale_x_date(date_labels = "%b") + 
  scale_color_manual(name = "Year", values = c("navy", "orange")) +
  labs(title = "Upper Release Discharge", y = "Mean Daily Flow (cfs)", 
       x = "Date") +
  theme_classic() + 
  theme(text = element_text(size=16))+
  theme( title = element_text(size=18), 
         legend.text= element_text(size= 12),
         legend.title = element_text(size = 14)) +
  ggsave("figure_output/environmental_data/Discharge/updated_Freemontflow.pdf", width = 10, height = 8)


# Upriver Flow, near 2017 Release location : DID NOT USE THIS ONE FOR PLOTS
